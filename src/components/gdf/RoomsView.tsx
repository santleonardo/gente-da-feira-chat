              ...
              {ROOM_ICONS.map((emoji) => (
                <button
                  ...
                  className={`h-9 w-9 rounded-lg text-lg flex items-center justify-center transition-colors ${
                    icon === emoji ? "bg-primary text-primary-foreground ring-2 ring-primary" : "bg-muted hover:bg-accent"
                  }`}
                >
                  {emoji}
                </button>
...
function RoomChat({ room, onBack, onRefreshRooms }: { room: any; onBack: () => void; onRefreshRooms: () => void }) {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      ...
      if (rmErr) {
        console.error("Client-side room_members error:", rmErr.message);
      }
          ...
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex gap-2 ${isMine ? "flex-row-reverse" : ""}`}>
                {!isMine && (
                  <UserAvatar user={{ id: sender.id || msg.sender_id, display_name: sender.display_name || "Usuário", avatar_url: sender.avatar_url }} className="h-7 w-7 shrink-0" />
                )}
                <div className={`max-w-[75%] ${isMine ? "text-right" : ""}`}>
                  {!isMine && <span className="text-[10px] font-medium text-muted-foreground">{sender.display_name || "Usuário"}</span>}
                  <div className={`rounded-2xl px-3.5 py-2 text-sm inline-block ${isMine ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
                    {msg.content}
                  </div>
      ...
      <div className="border-t pt-3 mt-2 space-y-2">
        {isMember ? (
          <>
            ...
            <Button variant="ghost" size="sm" onClick={handleLeave} className="w-full text-muted-foreground hover:text-destructive gap-1.5 text-xs">
