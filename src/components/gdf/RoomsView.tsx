...
export function RoomsView() {
  ...
  const fetchRooms = useCallback(async () => {
    try {
      ...
      console.error("[RoomsView] fetchRooms", err);
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <div className="space-y-2">
          ...
          {official.length === 0 && (
            <p className="text-sm text-muted-foreground py-4 text-center">
              Nenhuma sala oficial disponível
            </p>
          )}
        </div>
      </div>
...
function RoomCard({
  ...
  const memberCount = room._count?.members ?? room.member_count ?? 0;
...
function RoomChat({
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      console.error("[RoomChat] fetchMessages", err);
    } finally {
      setLoading(false);
    }
  }, [room.id]);
  ...
  // Supabase Realtime: escutar novas mensagens da sala
  useEffect(() => {
    ...
    const setupRealtime = () => {
      channel = supabase
        ...
        .on(
          ...
          async (payload) => {
            // Buscar a mensagem completa com sender
  ...
  useEffect(() => {
    if (profile) {
      fetch(`/api/rooms/${room.id}/join`, {
        ...
        body: JSON.stringify({ userId: profile.id, roomId: room.id }),
      }).catch((err) => console.error("[RoomChat] auto-join", err));
    }
  }, [profile, room.id]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (res.ok && data.message) {
        ...
        setInput(text); // Restaurar texto em caso de erro
      }
    } catch {
      ...
      <div className="flex items-center gap-3 border-b pb-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {room.description || `${memberCount} membros`}
          </p>
        </div>
      </div>
            ...
            >
              {!isMine && (
                <Avatar className="h-7 w-7 shrink-0">
                  <AvatarFallback
                    className={`${getAvatarColor(msg.sender.id)} text-[10px] text-white`}
                  >
